...
class V3Config:
    ...
    def validate(self) -> None:
        ...
        for reviewer in self.review_policy.reviewer_lanes:
            if reviewer not in lane_names:
                raise V3ConfigError(f"review_policy references unknown reviewer lane {reviewer!r}")
            lane = next(
                candidate for candidate in self.hermes_lanes.lanes if candidate.name == reviewer
            )
...
def _section_to_dict(value: Any) -> Any:
    if dataclasses.is_dataclass(value) and not isinstance(value, type):
        return {f.name: _section_to_dict(getattr(value, f.name)) for f in dataclasses.fields(value)}
...
def _section_from_dict(section: str, value: Any) -> Any:
    ...
    for f in fields(section_type):
        if f.name not in value:
            continue
        raw = value[f.name]
        nested = _NESTED_LIST_FIELDS.get((section, f.name))
        if nested is not None and isinstance(raw, list):
            raw = [_build_dataclass(nested, item) for item in raw]
        kwargs[f.name] = raw
    return _build_dataclass(section_type, kwargs)
